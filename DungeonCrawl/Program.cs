...
namespace DungeonCrawl
{
    ...
    enum PlayerTurnResult
    {
        ...
        NextLevel,
        BackToGame,
        OpenShop          // new result to open shop
    }
    ...
    internal class Program
    {
            ...
            while (state != GameState.Quit)
            {
                switch (state)
                {
                    case GameState.CharacterCreation:
                        ...
                        shop = new Shop(Vector2.Zero);
                        shop.RestockShop(random);
                        PlaceShopToMap(currentLevel, shop, random);
                    ...
                    case GameState.GameLoop:
                        ...
                        while (true)
                        {
                            ...
                            PlayerTurnResult result = DoPlayerTurn(currentLevel, player, monsters, items, dirtyTiles, messages, shop);
                            DrawInfo(player, monsters, items, messages);
                    ...
                    case GameState.Shop:
                        // Draw shop 
                        PlayerTurnResult shopResult = DrawShopMenu(player, shop, messages);
        ...
        static void DrawCommands()
        {
            ...
            Console.SetCursorPosition(cx, ln); ln++;
            Print("S", ConsoleColor.Magenta); Print(" = Shop", ConsoleColor.White);
        }
        ...
        static PlayerTurnResult DoPlayerTurn(Map level, PlayerCharacter character, List<Monster> enemies, List<Item> items, List<int> dirtyTiles, List<string> messages, Shop shop)
        {
        ...
        static int CalculateItemPrice(Item item)
        {
            ...
            {
                ItemType.Weapon => item.quality * 10,
                ItemType.Armor => item.quality * 8,
                ItemType.Potion => item.quality * 5,
                _ => 1
            };
        }
        ...
        // Method to draw the shop on the map
        static void DrawShop(Shop shop)
        {
            Console.SetCursorPosition((int)shop.position.X, (int)shop.position.Y);
            Print('S', ConsoleColor.Magenta);
        }



        // Method to handle the shop menu
        static PlayerTurnResult DrawShopMenu(PlayerCharacter character, Shop shop, List<string> messages)
        {
            Console.Clear();
            Console.SetCursorPosition(1, 1);
            PrintLine("Welcome to the Shop!", ConsoleColor.Magenta);
            PrintLine($"Your gold: {character.gold}", ConsoleColor.Yellow);
            PrintLine("Select item to buy by inputting the number next to it. Press ESC to exit.", ConsoleColor.White);
            ...
            for (int i = 0; i < shop.inventory.Count; i++)
            {
                ...
                Print($"{i} ", ConsoleColor.Cyan);
                Print($"{item.name} ", ConsoleColor.White);

                string typeDesc = item.type switch
                {
                    ItemType.Weapon => $"Damage: {item.quality}",
                    ItemType.Armor => $"Defense: {item.quality}",
                    ItemType.Potion => $"Healing: {item.quality}",
                    _ => ""
                };

                PrintLine($"({typeDesc}) - {price} gold", ConsoleColor.White);
            }

            while (true)
            {
                Print("Choose item to buy (or ESC to exit): ", ConsoleColor.Yellow);
                ConsoleKeyInfo key = Console.ReadKey();
                ...
                if (int.TryParse(key.KeyChar.ToString(), out selectionIndex))
                {
                    if (selectionIndex >= 0 && selectionIndex < shop.inventory.Count)
                    {
                        Item selectedItem = shop.inventory[selectionIndex];
                        int price = CalculateItemPrice(selectedItem);

                        if (character.gold >= price)
                        {
                            character.gold -= price;
                            // Create a copy of the item so we don't remove it from the shop
                            ...
                            GiveItem(character, boughtItem);
                            messages.Add($"You bought {selectedItem.name} for {price} gold.");
                            Console.WriteLine();
                            PrintLine($"You bought {selectedItem.name}!", ConsoleColor.Green);
                        }
                        else
                        {
                            Console.WriteLine();
                            PrintLine("Not enough gold!", ConsoleColor.Red);
                        }
                    }
                    else
                    {
                        Console.WriteLine();
                        PrintLine("Invalid selection", ConsoleColor.Red);
                    }
                }
                ...
                // Redraw the shop menu with updated gold value
                Console.Clear();
                Console.SetCursorPosition(1, 1);
                PrintLine("Welcome to the Shop!", ConsoleColor.Magenta);
                PrintLine($"Your gold: {character.gold}", ConsoleColor.Yellow);
